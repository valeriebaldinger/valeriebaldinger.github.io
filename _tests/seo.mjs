...
assert.match(home, /id="prompt-to-portfolio">From Prompt to Portfolio: <span>Generative AI and Retail Investment<\/span><\/h4>/);
assert.doesNotMatch(home, /href="[^"]*(?:papers\/from-prompt-to-portfolio|files\/From_Prompt_to_Portfolio\.pdf)/);
assert.ok(read(paperPage).includes(`rel="canonical" href="${latestVersionURL.href}"`));
const paperPDF = 'files/From_Prompt_to_Portfolio.pdf';
assert.ok(read(paperPage).includes(`http-equiv="refresh" content="0;url=/${paperPDF}"`));
assert.match(read(paperPage), /<body>\s*<\/body>/);
assert.doesNotMatch(read(paperPage), /The paper PDF is not yet available/);
assert.equal(fs.readFileSync(path.join(root, paperPDF)).subarray(0, 5).toString(), '%PDF-');
assert.ok(!urls.includes(new URL(paperPDF, canonical).href), 'Do not advertise a blank PDF in the sitemap.');
